...
const JS = new URL("../dist/contrast-checker.js", import.meta.url);
const CSS = new URL("../dist/contrast-checker.css", import.meta.url);
...
const foreign = [...new Set(selectors.map((s) => s.trim()))].filter((s) => !s.startsWith(".cc"));
check(foreign.length === 0, "defines only .cc- classes", foreign.join(" "));
